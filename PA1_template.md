...
The data file is downloaded if it is not present
It is uncompressed and read as a data frame.
...
Tabulate the average steps per interval for each dataset.
Plot the two datasets side by side for comparison.
